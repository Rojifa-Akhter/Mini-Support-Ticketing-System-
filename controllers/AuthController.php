...
class AuthController {
    ...
    public function register() {
        ...
        if (!in_array($input['role'], ['admin', 'agent'])) {
            ResponseHelper::json(['error' => 'Invalid role'], 400);
        }

        if (User::findByEmail($input['email'])) {
            ResponseHelper::json(['error' => 'Email already registered'], 400);
        }

        $user = User::create($input['name'], $input['email'], $input['password'], $input['role']);
        ResponseHelper::json(['message' => 'User registered', 'user' => ['id' => $user->id, 'name' => $user->name, 'email' => $user->email, 'role' => $user->role]]);
    }
    ...
    public function login() {
        ...
        ResponseHelper::json(['message' => 'Login successful', 'token' => $token]);
    }
